...
void sleep_and_signal(int time_green, int min_interval, pthread_mutex_t *mutex, 
            pthread_cond_t *conditional_variable, pthread_cond_t *conditional_variable_2);
int uniform_distribution(int rangeLow, int rangeHigh);
char *get_current_time();

void *print_lights(int color, char* directions);

void *print_vehicle_proceeding(int id, char* direction);
...
int main(int argc, char ** argv)
{
    ...
    pthread_mutex_init(&controller_1_mutex, NULL);
    pthread_mutex_init(&controller_2_mutex, NULL);
    pthread_mutex_init(&controller_3_mutex, NULL);
    ...
    pthread_mutex_destroy(&controller_1_mutex);
    pthread_mutex_destroy(&controller_2_mutex);
    pthread_mutex_destroy(&controller_3_mutex);
...
void * mini_controller_routine(void * arg)
{
    ...
    */
    if (id == 0) {
        print_lights(1 , directions);
        sleep_and_signal(time_green, min_interval, &controller_1_mutex, &vehicles_waiting_on_c1_n2s, &vehicles_waiting_on_c1_s2n);
        print_lights(0 , directions);
        ...
        while (1) {
            ...
            sleep(2);
            print_lights(1 , directions);
            sleep_and_signal(time_green, min_interval, &controller_2_mutex, &vehicles_waiting_on_c2_e2w, &vehicles_waiting_on_c2_w2e);
            print_lights(0 , directions);
        ...
        while (1) {
            ...
            sleep(2);
            print_lights(1 , directions);
            sleep_and_signal(time_green, min_interval, &controller_3_mutex, &vehicles_waiting_on_c3_n2w, &vehicles_waiting_on_c3_s2e);
            print_lights(0 , directions);
            pthread_mutex_lock(&green_mutex);
...
void * vehicle_routine(void * arg)
{
	...
	if (!strcmp("n2s", direction)) {
        //printf("n2s dir\n");
        pthread_mutex_lock(&controller_1_mutex);
		pthread_cond_wait(&vehicles_waiting_on_c1_n2s, &controller_1_mutex);
        while (1) {
            if (id == last_vehicle_n2s+1) {
                last_vehicle_n2s++;
                print_vehicle_proceeding(id, direction);
                pthread_mutex_unlock(&controller_1_mutex);
                break;
            } else {
                pthread_cond_wait(&vehicles_waiting_on_c1_n2s, &controller_1_mutex);
            }
        ...
        //printf("s2n dir\n");
        pthread_mutex_lock(&controller_1_mutex);
        pthread_cond_wait(&vehicles_waiting_on_c1_s2n, &controller_1_mutex);
        while (1) {
        ...
        //printf("e2w dir\n");
        pthread_mutex_lock(&controller_2_mutex);
        pthread_cond_wait(&vehicles_waiting_on_c2_e2w, &controller_2_mutex);
		while (1) {
            if (id == last_vehicle_e2w+1) {
                last_vehicle_e2w++;
                print_vehicle_proceeding(id, direction);
                pthread_mutex_unlock(&controller_2_mutex);
                break;
            } else {
                pthread_cond_wait(&vehicles_waiting_on_c2_e2w, &controller_2_mutex);
            }
        ...
        //printf("w2e dir\n");
        pthread_mutex_lock(&controller_2_mutex);
        pthread_cond_wait(&vehicles_waiting_on_c2_w2e, &controller_2_mutex);
		while (1) {
        ...
        //printf("n2w dir\n");
        pthread_mutex_lock(&controller_3_mutex);
        pthread_cond_wait(&vehicles_waiting_on_c3_n2w, &controller_3_mutex);
		while (1) {
            if (id == last_vehicle_n2w+1) {
                last_vehicle_n2w++;
                print_vehicle_proceeding(id, direction);
                pthread_mutex_unlock(&controller_3_mutex);
                break;
            } else {
                pthread_cond_wait(&vehicles_waiting_on_c3_n2w, &controller_3_mutex);
            }
        ...
        //printf("s2e dir\n");
        pthread_mutex_lock(&controller_3_mutex);
        pthread_cond_wait(&vehicles_waiting_on_c3_s2e, &controller_3_mutex);
		while (1) {
...
void sleep_and_signal(int time_green, int min_interval, pthread_mutex_t *mutex, 
            pthread_cond_t *conditional_variable, pthread_cond_t *conditional_variable_2) {
    unsigned int green_time = time_green * 1000 * 1000;
    unsigned int min_interval_micro = min_interval * 1000 * 1000;
    while (green_time > 0) {
            //printf("time green %d\n", time_green_function);
            pthread_mutex_lock(&(*mutex));
            int signal_1 = pthread_cond_signal(&(*conditional_variable));
            int signal_2 = pthread_cond_signal(&(*conditional_variable_2));
            if (!signal_1 || !signal_2)  {
                pthread_mutex_unlock(&(*mutex));
                if ((green_time - min_interval_micro) < 0) {
                    usleep(green_time);
                    green_time = 0;
                } else {
                    usleep(min_interval_micro);
                    green_time = green_time - min_interval_micro;
                }
                //printf("time green left: %d\n", time_green_function);
            } else {
                pthread_mutex_unlock(&(*mutex));
                unsigned int microseconds_sleep = 5000;
                usleep(microseconds_sleep);
                green_time = green_time - 50;
            }
        }
    return;
}
